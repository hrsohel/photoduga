...
export default function LeftSide({activeLeftBar, setActiveLeftBar, setShowPageNavigation}) {
    return (
        <div className='flex items-center justify-start flex-col w-[130px] bg-white h-[91vh] sticky top-0'>
            {
                leftSideData.map(data => (
                    <div onClick={() => {
                        setActiveLeftBar(data.label);
                        setShowPageNavigation(data.label === "Frames");
                    }} key={data.id} 
